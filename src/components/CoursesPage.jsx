    ...
    <div style={{ fontFamily: "'Georgia', 'Times New Roman', serif", minHeight: "100vh", background: "#f8f8f6" }}>
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
          ...
          <h1 style={{ color: "#fff", margin: 0, fontSize: isMobile ? "20px" : "28px", fontFamily: "'Trebuchet MS', sans-serif" }}>All Cisco Courses</h1>
        </div>
        <div style={{ background: "rgba(58,191,148,0.2)", padding: "8px 16px", borderRadius: "20px" }}>
          <span style={{ color: "#3abf94", fontFamily: "'Trebuchet MS', sans-serif", fontWeight: 600 }}>{courses.length} Courses Available</span>
        </div>
      </div>
{/* Premium Hero Banner */}
<div
  style={{
    background:
      "linear-gradient(135deg, #0f172a 0%, #1e3a8a 50%, #3abf94 100%)",
...
>
  ...
  >
    ...
    >
      Learn CCNA, CCNP, CCIE, Security, Linux and Network
      Automation through hands-on labs, video lessons,
...
>
  <div>
    <h2 style={{ color: "#3abf94", margin: 0 }}>15+</h2>
    <p>Courses</p>
  </div>

  <div>
    <h2 style={{ color: "#3abf94", margin: 0 }}>50K+</h2>
    <p>Students</p>
  </div>

  <div>
    <h2 style={{ color: "#3abf94", margin: 0 }}>2000+</h2>
    <p>Labs</p>
  </div>

  <div>
    <h2 style={{ color: "#3abf94", margin: 0 }}>4.9★</h2>
    <p>Rating</p>
  </div>
</div>
        ...
        <div style={{ padding: isMobile ? "32px 16px" : "48px 40px", background: "#f0f2f5" }}>
          <div style={{ maxWidth: "1200px", margin: "0 auto" }}>
            <div style={{ display: "flex", alignItems: "center", gap: "12px", marginBottom: "32px" }}>
              ...
              <h2 style={{ margin: 0, fontSize: isMobile ? "24px" : "32px", color: "#1a1a2e" }}>Featured Courses</h2>
            </div>
            <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "repeat(3, 1fr)", gap: "24px" }}>
              {featuredCourses.map(course => (
                <CourseCard key={course.id} course={course} isMobile={isMobile} hovered={hoveredCourse === course.id} onHover={setHoveredCourse} levelColors={levelColors} />
              ))}
            </div>
          </div>
        </div>
        ...
        <div style={{ maxWidth: "1200px", margin: "0 auto" }}>
          {selectedCategory !== "all" && (
            <div style={{ marginBottom: "32px" }}>
              <h2 style={{ fontSize: isMobile ? "22px" : "28px", color: "#1a1a2e" }}>{categories.find(c => c.id === selectedCategory)?.name} Courses</h2>
              <p style={{ color: "#666", marginTop: "8px" }}>{filteredCourses.length} courses available</p>
            </div>
          )}
          ...
          {filteredCourses.length === 0 && (
            <div style={{ textAlign: "center", padding: "60px" }}>
              <p style={{ fontSize: "18px", color: "#999" }}>No courses found in this category.</p>
            </div>
          )}
        </div>
      </div>
      ...
      <div style={{
        ...
        <p style={{ color: "#ccc", fontSize: isMobile ? "14px" : "16px", marginBottom: "32px", maxWidth: "600px", marginLeft: "auto", marginRight: "auto" }}>
...
function CourseCard({ course, isMobile, hovered, onHover, levelColors }) {
  return (
    <div
  ...
  >
    🎥 {course.lessons} Videos
      ...
      <div style={{ padding: "20px" }}>
        ...
        <button
          style={{ width: "100%", background: course.color, color: "#fff", border: "none", borderRadius: "10px", padding: "12px", fontFamily: "'Trebuchet MS', sans-serif", fontWeight: 600, cursor: "pointer", transition: "opacity 0.2s" }}
